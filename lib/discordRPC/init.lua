--[[
	...
	modified for FNF-Aster
]]

local OS, ffi = love.system.getOS(), require "ffi"

local tryload
function tryload(dir, file, ...)
	local s, module = pcall(ffi.load, dir:replace('.', '/') .. '/' .. file)
	if not s then s, module = pcall(ffi.load, file) end
	if s then
		return module
	else
...
function discordRPC.initialize(applicationId, autoRegister, optionalSteamId)
	...
	lib.Discord_Initialize(applicationId, eventHandlers,
...
function discordRPC.updatePresence(presence)
	...
	lib.Discord_UpdatePresence(cpresence)
end
...
function discordRPC.respond(userId, reply)
	...
	lib.Discord_Respond(userId, replyMap[reply])
end
